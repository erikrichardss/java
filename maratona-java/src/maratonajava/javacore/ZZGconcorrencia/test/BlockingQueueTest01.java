...
public class BlockingQueueTest01 {
    ...
    static class RemoveFromQueue implements  Runnable {
        ...
        @Override
        public void run() {
            System.out.printf("%s going to sleep for 2s %s %n", Thread.currentThread().getName(), bq.peek());
            try {
                TimeUnit.SECONDS.sleep(5);
                System.out.printf("%s removing value from queue %s %n", Thread.currentThread().getName(), bq.peek());
